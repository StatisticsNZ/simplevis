...
#' #' @param col_labels A function or vector to modify colour scale labels, as per the ggplot2 labels argument in ggplot2 scales functions. If NULL, categorical variable labels are converted to sentence case. Use ggplot2::waiver() to keep colour labels untransformed.
...
#'     if (is.null(pal)) pal <- viridis::viridis(col_n)
#'     else pal <- pal[1:col_n]
#'     if (pal_rev == TRUE) pal <- rev(pal)
#'     if (is.null(col_labels)) col_labels <- scales::number_format()
