...
#' @param y_breaks_n For a numeric or date y variable, the desired number of intervals on the y scale, as calculated by the pretty algorithm. Defaults to 6. 
...
gg_hbar_col <- function(data,
  ...
  if (mobile == FALSE) {
    plot <- plot +
      ...
      guides(fill = guide_legend(byrow = TRUE, reverse = legend_reverse), col = guide_legend(byrow = TRUE, reverse = legend_reverse))
  }
  else if (mobile == TRUE) {
    plot <- plot +
      labs(
      ...
      guides(fill = guide_legend(ncol = 1, reverse = legend_reverse), col = guide_legend(ncol = 1, reverse = legend_reverse)) +
      theme_mobile_extra()
  }
