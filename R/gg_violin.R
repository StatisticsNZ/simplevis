...
#' #' @param alpha_fill The opacity of the points. 
...
#'     theme +
#'     geom_violin(aes(!!x_var, !!y_var, group = !!x_var), size = size_line, col = pal_line, fill = pal_fill) 
#' 
#'   #x scale
...
#'   pal_fill <- scales::alpha(pal, alpha = alpha_fill)
#'   pal_na_point <- scales::alpha(pal_na, alpha = alpha_fill)
#'   
#'   #fundamentals
#'   plot <- ggplot(data) +
#'     coord_cartesian(clip = "off") +
...
#'   
#'   plot <- plot +
#'     geom_violin(aes(!!x_var, !!y_var, col = !!col_var), size = size_line) 
#' 
#'   #x scale
#'   if (is.numeric(x_var_vctr) | lubridate::is.Date(x_var_vctr) | lubridate::is.POSIXt(x_var_vctr)) {
#'     
...
#'     coord_cartesian(clip = "off") +
#'     theme +
#'     geom_violin(aes(!!x_var, !!y_var, col = !!col_var, group = !!col_var), size = size_line) 
#' 
#'   #x scale 
#'   if (is.character(x_var_vctr) | is.factor(x_var_vctr)){
#'     if (is.null(x_expand)) x_expand <- waiver()
