...
#' @param x_zero_mid Add balance to the x axis so that zero is in the centre of the x scale. Only applicable where facet_scales equals "fixed" or "free_y".
...
#' @keywords internal
sv_x_zero_adjust <- function(x_var_vctr, x_zero_mid, x_zero, x_zero_line) {
  ...
  if(is.null(x_zero_line)) {
    if(x_above_and_below_zero == TRUE | x_zero_mid == TRUE) x_zero_line <- TRUE
...
#' @param y_zero_mid Add balance to the y axis so that zero is in the centre of the y scale. Only applicable where facet_scales equals "fixed" or "free_y".
...
#' @keywords internal
sv_y_zero_adjust <- function(y_var_vctr, y_zero_mid, y_zero, y_zero_line) {
  ...
  if(is.null(y_zero_line)) {
    if(y_above_and_below_zero == TRUE | y_zero_mid == TRUE) y_zero_line <- TRUE
