...
```{r}
gg_point(penguins, bill_length_mm, body_mass_g, pal = "#da3490")
...
```{r}
gg_point_col(penguins, bill_length_mm, body_mass_g, col_var = species)
```

You can adjust the colours of the coloured variable using the pal argument as described earlier.

```{r}
gg_point_col(penguins, bill_length_mm, body_mass_g, species, pal = c("#da3490", "#9089fa", "#47e26f"))
```

The `pals` package provides a large amount of colour palettes.

```{r}
gg_point_col(penguins, bill_length_mm, body_mass_g, species, pal = pals::brewer.dark2(3))
```
...
gg_point_col(plot_data, carat, price, z,
                 col_method = "quantile",
...
From simplevis 4.1.0 onwards, default colour palettes are:

```{r}
scales::show_col(pal_viridis_reorder(1))
```
