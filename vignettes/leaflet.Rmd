...
```{r}
leaflet_sf_col(example_sf_point, median, 
              col_method = "quantile",
...
The popup will default to a `leafpop::popupTable` of all variables, but popups can be adjusted to a subset of column using the `popup_vars_vctr` argument.
...
The tooltip will default to the colour variable, but can be adjusted using the `text_var` variable. It has been called the `text_var` to align with the term that `ggplot2` calls the hover value or tooltip.
