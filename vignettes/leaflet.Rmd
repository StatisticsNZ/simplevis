...
Legend labels are not as flexible as in the ggplot2 wrapper functions. You can adjust by using the `col_labels` argument, and providing a manual vector of breaks. The `col_label_digits` can also provide a shortcut to round the labels.
...
leaflet_sf_col(example_sf_point, 
               col_var = trend_category) %>% 
  leaflet::addPolygons(data = example_sf_borders, 
                       color = "#35B779", 
                       weight = 3, 
                       fillOpacity = 0, 
                       opacity = 1)
