...
```{r, message = FALSE, warning = FALSE, fig.height = 4, fig.width = 6}
iris %>% 
  ggplot_scatter(Sepal.Width, Sepal.Length)
...
ggplot_scatter(iris, Sepal.Width, Sepal.Length,
               ...
               x_title = "Sepal width (cm)",
               y_title = "Sepal length (cm)",
               wrap_title = 10, wrap_x_title = 5, wrap_y_title = 5)
```
...
Adjust the x labels, and change legend columns to 1.
...
Manually adjust the legend labels for the colour variable.
...
ggplot_vbar_col(plot_data, year, wind, status,
...
ggplot_hbar(plot_data, average_price, cut, x_na_bar = TRUE)
```
...
Make a non-facetted plot for a mobile device. Note the template shiny apps provide a function to detect what device the user is on and returns a TRUE or FALSE value that you can draw on with isMobile = input$isMobile.
...
```{r, message = FALSE, warning = FALSE, fig.height = 4, fig.width = 6}
plot <- ggplot_scatter(iris, Sepal.Width, Sepal.Length, tip_var = Petal.Width)

plotly::ggplotly(plot, tooltip = "text")

```

Make a ggplotly interactive plot with custom tooltip that can be created with the add_tip function. Note that this function will convert snakecase column names to sentence case with underscores changed to spaces. The ggplotly argument tooltip = "text" is required to implement this in combination with the tip_var argument in the simplevis function. 
