...
### Making mobile compliant ggplots

`simplevis` supports non-facetted mobile compliant ggplot visualisation by providing an argument called `isMobile`. 

To incorporate this into an app, you need in your app: 

* the `mobileDetect` function in the `ui.R` in the app templates
* the `www/js/mobile.js` script
* simplevis ggplot functions with argument `isMobile = input$isMobile`
...
ggplot_hbar(plot_data, average_price, cut, isMobile = FALSE)
```

```{r, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 3.5}
plot_data <- ggplot2::diamonds %>%
  mutate(cut = stringr::str_to_sentence(cut)) %>%
  group_by(cut) %>%
  summarise(average_price = mean(price)) %>%
...
ggplot_hbar(plot_data, average_price, cut, isMobile = TRUE)
```
