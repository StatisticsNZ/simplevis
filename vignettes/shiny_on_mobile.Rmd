...
If you are intending to publish a `shiny` app with `ggplot` based graphs in it that are viewed by users on mobile devices, this will present challenges.

`simplevis` offers methods to support making ggplot objects that work on either non-mobile or mobile device throught the `isMobile` argument.

### Graphs that work in apps on mobile devices
...
`simplevis` provides mobile support for  plots through a `isMobile` argument, which makes the above changes. 
...
### How to incorporate this into a shiny app
...
* in the `server.R`, generate a `reactive` plot object incorporating an `isMobile = input$isMobile` argument into `simplevis` ggplot function
