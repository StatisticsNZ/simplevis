...
simplevis emphasises cognitive ease and speed, and subsequently some flexibility is lost compared to ggplot2. 
...
* horizontal bar (`gg_hbar`)
* vertical bar (`gg_vbar`)
* line plot (`gg_line`)
* scatter graph (`gg_point`)
* boxplot (`gg_boxplot`)
...
gg_line(plot_data, year, y_var = wind)
```
...
gg_hbar_facet(plot_data, average_price, cut, clarity)
```
...
gg_sf(example_sf_point, size_point = 0.25)
```

```{r, message = FALSE, warning = FALSE, fig.height = 3.5, fig.width = 7}
nz

gg_sf(example_sf_point, borders = nz, size_point = 0.25)
```
...
```{r, message = FALSE, warning = FALSE, fig.height = 3.5, fig.width = 7}
gg_sf_facet(example_sf_point, trend_category, 
                borders = nz, 
                size_point = 0.5)
```
...
tibble::tribble(
  ...
  "line", "numeric, date or categorical", "numeric", "categorical or numeric", "categorical", "identity",
  "point", "numeric, date or categorical", "numeric", "categorical or numeric", "categorical", "identity",
  "boxplot", "numeric, date or categorical", "numeric", "categorical", "categorical", "boxplot or identity",
...
```{r, eval = FALSE}
plot <- gg_point(iris, Sepal.Width, Sepal.Length)
...
```{r, message = FALSE, warning = FALSE, fig.height = 3.5, fig.width = 7}
gg_point_col(iris, Sepal.Width, Sepal.Length, Species, 
...
For further information, see the [shiny article](https://statisticsnz.github.io/simplevis/articles/shiny.html).