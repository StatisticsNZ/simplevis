...
* facetted, but_ _not_ coloured by a variable (`*_facet()`)
...
gg_line_col(plot_data, 
           ...
           col_var = status,
           x_breaks_n = 10,
           x_labels = ~stringr::str_sub(.x, 3, 4),
...
plot_data <- penguins %>%
  filter(!is.na(sex)) %>% 
  group_by(sex) %>%
  summarise(body_mass_g_p50 = median(body_mass_g, na.rm = TRUE), 
            body_mass_g_p05 = quantile(body_mass_g, probs = 0.05, na.rm = TRUE), 
            body_mass_g_p95 = quantile(body_mass_g, probs = 0.95, na.rm = TRUE))

gg_bar(plot_data,
       x_var = sex,
       y_var = body_mass_g_p50,
       y_title = "Body mass g") +
  ggplot2::geom_text(ggplot2::aes(x = sex, y = body_mass_g_p05 - 500, label = body_mass_g_p50),
                     col = "white") +
  ggplot2::geom_errorbar(ggplot2::aes(x = sex, ymin = body_mass_g_p05, ymax = body_mass_g_p95), 
                         width = 0.2) +
  ggplot2::scale_y_continuous(
    name = "Body mass g",
    breaks = ~pretty(.x, 10),
    limits = ~c(min(pretty(.x, 10)), max(pretty(.x, 10))),
    expand = c(0, 0)
  )
```
