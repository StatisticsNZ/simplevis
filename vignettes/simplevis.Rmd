...
These `ggplot` graphs have been designed that users can convert them easily to html interactive objects by wrapping them in `plotly::gglotly(plot)`. A customised tooltip can be provided using the `tip_var` argument in `simplevis` functions with `plotly::gglotly(plot, tooltip = "text)`. Automated tip_text columns can be created using the `add_plot_tip` function.
...
* In `server.R`, add code within reactive plot_data and plot components, as well as equivalent map and table components.
