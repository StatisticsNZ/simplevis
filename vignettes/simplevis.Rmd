...
The order of variables is always `x_var`, `y_var`, `col_var` and `facet_var`. 

So, once familiar with the package, you may choose not to name the arguments that these variable work with. 
...
simplevis also provides leaflet wrapper functions for `sf` and `stars` objects. These functions work in a similar way to the `gg_sf*()` and `gg_stars*()` functions, but have a `leaf_` prefix. Note there is no borders argument available in the `leaf_*()` functions.
