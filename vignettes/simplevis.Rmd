...
* minimises aesthetics available to x, y, colour and facetting
* simplifies colouring method for users 
* uses the power of consistent prefixes and the Rstudio auto-complete 
* uses ggplot2 based terminology for leaflet visualisation
...
They work in essentially the same way as the `ggplot` non-facetted `sf` functions. Arguments have been aligned with simplevis ggplot wrapping functions (e.g. size_point = radius, line_size = weight, alpha = fillOpacity).
...
Because both of the ggplot and leaflet wrapper functions produce ggplot and leaflet objects, you can add layers in the same way that you would normally to these objects. Be careful to use `+` to add layers to ggplot objects and `%>%` to add to leaflet objects. 
...
For further information, see the [shiny_on_mobile article](https://statisticsnz.github.io/simplevis/articles/shiny_on_mobile.html).