...
`simplevis` also offers more customisability and ease for making tooltips (i.e. hover values). The `mutate_text` functions  produces a variable named `text` that can be added into the simplevis function through `text_var`, and then visualised in the `ggplotly` object if `tooltip = "text"` if added to the function. 

`mutate_text` creates a tooltip column of all variables and values in the data by default, but you can express a subset of these by specifying a vector of variable names to the function.
...
gg_vbar(plot_data, sex, body_mass_g, y_pretty_n = 3, x_na = FALSE) +
...
It also enables you to 'hack' some other `geoms` of `ggplot2` currently unsupported by simplevis, while utilising the functionality and look of `simplevis`. 

```{r}
gg_point(penguins, species, body_mass_g, size_point = NA, y_zero = T, y_labels = scales::comma) +
  geom_violin(aes(species, body_mass_g)) +
